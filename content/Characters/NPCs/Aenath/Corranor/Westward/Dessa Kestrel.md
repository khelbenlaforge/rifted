---
title: Dessa Kestrel
aliases: []
tags:
- npc
- creature/humanoid
campaign: Rifted
introduced: 2026-09-23
secret: false
state: alive
updated: 2026-09-26T00:15:02
created: 2026-09-23T18:28:40
---

# Dessa Kestrel

> [!infobox|right wikipedia]+
> # Dessa Kestrel
> ###### Quick Reference
> | | |
> | ---- | --- |
> | Race | Elf |
> | Class | Cleric (Knowledge Domain) |
> | Location | [[Westward]] (The Cliffwatch) |
> | Affiliation | [[Adamantine Guild]] |

Race:: Elf
Subrace::
Pronouns:: she/her
Class:: Cleric (Knowledge Domain)
Location:: [[Westward]] (The Cliffwatch)
Affiliation:: [[Adamantine Guild]]
Deity:: [[Oghma]]

## Description

Old for an elf, and it shows only where she lets it: fine creases at the eyes from decades of reading fine print by bad light, a patient face with no warmth spared on it. Auburn hair, going nowhere near grey; skin a deep, even gold. Small reading glasses she takes off and puts back on like punctuation, worn over plain scholar's robes rather than any Guild uniform. Her hands are the tell: ink at the second knuckle on the right that never fully scrubs out, a callus worn where a stylus sits more often than a hilt. A shortsword and a bow slung across her back, Guild-issue, clean-kept and seldom drawn. What she actually reaches for first is the wax-sealed ledger satchel at her hip and the short marked baton she carries more often than either weapon, using it to indicate things: a doorway, a discrepancy, a target. Her eyes move over a room once, top to bottom, the way an auditor's eyes move over a page before they trust a single number written on it.

## Personality

Methodical to the point of dryness. She doesn't raise her voice to make a point; she just states the count and lets the number do the arguing. Years of reading other people's paperwork gave her a habit of narrating discrepancies out loud, half for herself and half for whoever's listening, and she's never quite broken it even in a fight. She reads an evaluation the same way she used to read a ledger that didn't balance: not by what a recruit says about themselves, but by what they do once the numbers stop cooperating.

## Background

Dessa spent a decade reading other branches' books before Westward ever had a branch of its own to read: recruitment logs, incident reports, the quiet paperwork that tells the Guild which evaluations were thorough and which ones just stamped a bracer and hoped. When the Westward posting opened, she asked for it herself. Westward needs a Guild presence precisely because of what sits under the island when it lifts, and an evaluation that doesn't actually test a recruit for that kind of dark is worth exactly what the paper it's written on is worth. The Cliffwatch itself watches the stair below it more closely than it watches its own ledgers, by her own accounting, and evening that out has been most of her job since she took the post. She's been down into the Keel exactly once, years ago, long enough to write the report herself rather than read someone else's version of it. She doesn't bring it up unless asked, and she never tells it the same way twice.

## Role in the Story

First point of contact for The AUGs' Adamantine Guild standing at The Cliffwatch in [[Westward]] (Session 3). Confirms the party's already-worn brass bracers as real rank rather than issuing fresh ones, then personally runs their Guild evaluation in the branch's own pocket-dimension testing ground, the Proving Floor. Same structural role as [[Rhona Voss]]'s evaluation of the Understudies and [[Petra Hale]]'s of the JULYs, played in Dessa's own register: measured, unhurried, closer to a survey report than a war story.

## D&D 5e Stat Block

*Encounter stat block, Dessa is an intended Guild-evaluation combat encounter, same function as [[Rhona Voss]]'s and [[Petra Hale]]'s builds, and the first of the three with real spellcasting. Mixed from three real sources via `query_5etools.py`: the **Scout Captain (XMM, CR 3)** supplies the martial chassis (AC, attack math, `Aim` bonus action, Shortsword/Longbow Multiattack; climb speed dropped, Stealth/Survival relabeled Insight/Investigation with values unchanged, to keep the flavor auditor-coded rather than field-scout-coded), distinct from Rhona's arena-fighter Gladiator base and Petra's escort-guard Guard Captain base. The **Warlord (VGM/MPMM, CR 12 as printed)** supplies **Indomitable (3/Day)** as her defensive trait (a reroll, not a guaranteed success, a genuine mechanical difference from Rhona's and Petra's Legendary Resistance) and the **Weapon Attack** legendary action (its ally-buffing **Command Ally** was dropped as irrelevant to a solo fight). The **Warlord's own Frighten Foe legendary action is replaced by Command**, a genuine 2024 spell of the same 2-action weight. Her `Spellcasting` trait is structurally modeled on **Viktra Mordenheim's (RHW, CR 7)** own spellcasting block, a real donor's `type: spellcasting` header-plus-daily-list format, with Viktra's construct-summoning spell swapped for **Bane**, the real-spell-for-fit substitution `stat-blocks.md` already documents. Her own `Uncanny Dodge` reaction (inherited from the Scout Captain chassis) was dropped so she doesn't triple up on Petra's identical reaction. See DM Notes below for full build rationale and encounter-math calibration.*



### Lair Actions, the Proving Floor

*On initiative count 20 (losing all ties), the Proving Floor itself takes one lair action to cause one of the following effects. The same effect can't be used two rounds in a row.*

- **Rising Stair**: a section of floor grinds upward into a stepped rise, or drops into a shallow trench. Each creature standing on that section when it moves must succeed on a DC 13 Dexterity saving throw or fall Prone. Dessa built this room; she already knows where solid footing will be before the floor finishes moving.
- **Closing Wall**: a section of wall slides across the room. Dessa chooses one creature within 30 feet of her; that creature loses line of sight to every other creature in the room until the start of her next turn.
- **Failing Light**: the lanterns dim across a 20-foot-radius area of Dessa's choice. Ranged attack rolls made against a target inside that area have Disadvantage unless the attacker can see in darkness.

## Character Sheet

| | |
|---|---|
| **Level / CR** | Legendary NPC, CR 6 (estimate; Scout Captain/Warlord chassis mix plus Bane/Command spellcasting modeled on Viktra Mordenheim's Spellcasting trait, XMM/VGM-MPMM/RHW/XPHB, see DM Notes for calibration caveat) |
| **Race** | Elf |
| **HP (Max)** | 112 (15d8+45) |
| **AC** | 15 |
| **Speed** | 30 ft. |
| **Proficiency Bonus** | +2 (kept verbatim from Scout Captain's CR-3 chassis; see DM Notes) |

### Ability Scores

| STR | DEX | CON | INT | WIS | CHA |
|:---:|:---:|:---:|:---:|:---:|:---:|
| 11 (+0) | 16 (+3) | 12 (+1) | 14 (+2) | 15 (+2) | 11 (+0) |

### Saving Throws

Dexterity +5 · Intelligence +4

### Skills

Perception +6 · Insight +7 · Investigation +6

**Damage Immunities:** None.
**Condition Immunities:** None.
**Senses:** passive Perception 16
**Languages:** Common
**Equipment:** Shortsword, Longbow, a wax-sealed ledger satchel, a short marked baton

---

### Actions

| Name | Atk / Save | Range | Damage / Effect | Recharge |
|------|------------|-------|-----------------|----------|
| **Multiattack** | n/a | n/a | Make two attacks, Shortsword or Longbow in any combination | n/a |
| **Shortsword** | M +5 | 5 ft. | 6 (1d6+3) piercing; +10 (3d6) piercing if made with Advantage | n/a |
| **Longbow** | R +5 | 150/600 ft. (usable at any range inside the Proving Floor) | 7 (1d8+3) piercing; +10 (3d6) piercing if made with Advantage | n/a |
| **Bane** | DC 12 CHA | 30 ft. | Up to 3 creatures; each failed target subtracts 1d4 from every attack roll/save it makes until the spell ends | 2/Day, replaces Multiattack |

Bane is Concentration, 1 minute; granted by her Spellcasting trait, see Features below.

### Bonus Actions

| Name | Effect |
|------|--------|
| **Aim** | Advantage on the next attack roll made this turn |

### Legendary Actions

*Dessa has 3 legendary action points, spent on the options below at the end of another creature's turn. Only one option at a time; she regains all spent points at the start of her turn.*

| Name | Effect | Cost / Limit |
|------|--------|--------------|
| **Weapon Attack** | Makes one Shortsword or Longbow attack | 1 action |
| **Command** | One creature within 60 ft., DC 12 WIS save or follows a one-word command (Approach/Drop/Flee/Grovel/Halt) on its next turn | 2 actions |

### Lair Actions (initiative count 20, no repeats two rounds running)

| Name | Effect |
|------|--------|
| **Rising Stair** | Floor section shifts; DC 13 DEX save or fall Prone |
| **Closing Wall** | One creature within 30 ft. loses line of sight to every other creature in the room until Dessa's next turn |
| **Failing Light** | Ranged attacks into a 20-ft area have Disadvantage unless the attacker can see in darkness |

---

### Features

- **Indomitable (3/Day)**: if Dessa fails a saving throw, she can reroll it; she must use the new roll.
- **Aim** (Bonus Action): see Bonus Actions.
- **Spellcasting**: Wisdom-based, spell save DC 12, no material components needed. Bane (2/Day, cast in place of Multiattack) and Command (legendary action, see Legendary Actions) are her only spells.

## Session 3 Update

- Met The AUGs—[[Mike Kanbei]], [[Ruan Myrt Zaltana]], [[Rachel Tan]], and [[Rismawati Bayu]]—for the first time at the Cliffwatch.
- Confirmed the party's existing brass [[Adamantine Guild]] bracers as genuine rank; did not issue new bracers.
- Showed the party the branch's pocket-dimension personal rooms.
- Relayed secondhand reports: the [[Ether Scryer's Academy]] Antiquities department's mystery of no memory before roughly two hundred years; [[The Claret]]'s blood trade; a claim that [[Layla]] once worked under [[Garm]] before an experimental blood operation and her disappearance; and that Rachel Tan is directly a blood-trade target.
- Offered a future introduction to the [[Azure Tamers]] for the party's two druids, Ruan Myrt Zaltana and Rismawati Bayu.
- Told the party that Level 4 was open to claim.
- Began, but did not complete, a Guild evaluation on the Proving Floor; the party entered, no combat occurred, and the session ended.

