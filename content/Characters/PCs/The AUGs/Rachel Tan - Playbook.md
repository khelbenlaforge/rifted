...
*Companion to [[Rachel Tan]] — regenerated from the synced Character Sheet on 2026-08-26. Rerun this skill after any character-sheet-sync update to keep it current.*

*No Loadout Selection section: Rachel has no spell slots at all (Spell Slots: None). Fire Bolt, Thaumaturgy, and Hellish Rebuke are fixed racial grants, not a prepared list she chooses from.*

## Combat Decision Flowchart

**Triage first — check every turn, and on the trigger itself for the reactive items below:**
- Rachel fails a Strength check or saving throw → **Didn't Ask Permission** triggers automatically, no action: add her Rage damage bonus for her Barbarian level (+2 at level 3) to the roll after seeing the result — works whether or not she's currently Raging. Free, always take it.
- Rachel takes damage from a creature within 60 ft, **and she is not currently Raging** → **Hellish Rebuke** (Reaction, 1/Long Rest, DEX DC 11, 60 ft, 2d10 fire, ½ on success). This is only available on rounds she isn't Raging — Rage's own rules explicitly forbid casting spells or maintaining Concentration while active *(confirmed via a live lookup on the Rage feature — the sheet's compressed cell doesn't mention this restriction, and it matters because Raging is her main combat mode)*. If she's already Raging when hit, this Reaction simply isn't usable that round.

**Bonus Action** (check this before the Action below, since it changes what Action options are even legal):
- Not yet Raging, and a real fight is starting → **Rage** (3/Long Rest, self, 10 min max) — resistance to Bludgeoning/Piercing/Slashing, +2 Strength melee/thrown damage, advantage on Strength checks/saves. This also locks out Fire Bolt, Thaumaturgy, and Hellish Rebuke for its duration. Rage only lasts until the end of her next turn unless extended — attacking, forcing a save, or spending another Bonus Action each round keeps it going, which happens naturally in most fights
- Already Raging and mid-fight → no other Bonus Action option exists on the sheet; this slot is otherwise unused
...
- Nothing else fits → **Unarmed Strike** (M +6, 1+4 bludgeon) — weakest option, no Weapon Mastery property attached

**Reaction:** Hellish Rebuke (see Triage) — the only one on the sheet, and only while not Raging.
...
- Need an edge on an intimidation attempt → **Thaumaturgy's Booming Voice effect** *(RAW: voice booms up to three times louder for 1 min, granting advantage on Charisma (Intimidation) checks — sourced from a live spell lookup, since the sheet's cell just says "minor magical effect")*
- Need to tell when someone is being controlled, pressured, or told what to think by someone else → **Not Her Voice** *(RAW: advantage on Insight checks for this)*
- Need to lean on raw physical power for a check that wouldn't normally use Strength, while Raging → **Primal Knowledge** *(RAW: while Raging, use Strength for certain skill checks, and gain proficiency in one such skill for that Rage)*
- Need to shrug off a trap, environmental hazard, or reflex-based danger → **Danger Sense** *(RAW: advantage on Dexterity saves unless Incapacitated)*
- Need to restrain and hold a captive → **Manacles** *(Inventory)*
- Need to break down a door or barricade → **Portable Ram** *(Inventory)*, backed by **Athletics +6**
