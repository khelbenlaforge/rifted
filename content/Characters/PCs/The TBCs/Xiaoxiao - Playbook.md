...
**Action:**
- Undead or Fiend in range → **Holy Water** (thrown, DEX save DC 11, 2d8 radiant on a failed save) — limited consumable, save it for the matching creature type
- Single tough target at range → **Chromatic Orb** (R +6, 90 ft, 3d8 of a chosen damage type; matching-type dice can leap to a second target) — costs a spell slot
- Melee, setting up a Bonus Action smite → **Longsword** *(Sap, M +6, 5 ft, 1d8+4 slash — on hit, target has disadvantage on its next attack roll)*
- Ranged, want to slow a target down → **Javelin** *(Slow, M/R +6, 5 ft./30/120 ft., 1d6+4 pierce — on a damaging hit, reduce its Speed by 10 ft. until the start of your next turn)*
- Need to stop a melee target's opportunity attack → **Shocking Grasp** (M +6, touch, 1d8 lightning; target can't take opportunity attacks until its next turn)
- Nothing better to do with the Action → **Unarmed Strike** (M +6, 5 ft, 1+4 bludgeoning) — at-will fallback, costs nothing
- First hit each turn against a creature within 5 ft. of an ally → **By the Book** *(Cabin Ready, passive)* adds +1d6 radiant automatically — no separate choice, just land the hit
...
- Need intel before committing → **Divine Sense** (self, 60 ft, detect Celestials/Fiends/Undead/consecrated-desecrated ground for 10 min) — costs a Channel Divinity charge, shares the pool with Elemental Smite
- One shot to disrupt a hostile before it acts, or free a willing ally from Charmed/Frightened/Poisoned → **Please Remain Seated** *(Cabin Ready, WIS DC 14, 30 ft, 1/Short Rest)* — pick one effect per use

**Reaction:**
- Ally (or Xiaoxiao) falling → Feather Fall (see Triage)

**Movement:**
- Close to melee when a smite combo is worth setting up; hold at range with Chromatic Orb/Javelin when the fight allows it instead.
...
*Paladin prepares from its full class spell list and can swap the whole list after every Long Rest — pick the set below that matches the next expedition. Chromatic Orb, Divine Smite, Elementalism, and Thunderous Smite are always prepared via Genie Spells/Paladin's Smite and don't count against the swappable slots below.*

**Dungeon / combat-heavy:**
- Swap in: *Bless* — party-wide attack/save buff for a sustained fight; *Compelled Duel* — pull a dangerous target's attention off a squishier ally
- Keep: *Wrathful Smite* — control option is worth holding onto underground where retreat is harder
...
*Candidates pulled from the full 2024 Paladin spell list at 1st level and cantrip (query_5etools.py) — not just what's currently on the sheet.*
...
- Need to make a threat land without swinging first → **Intimidation +4**
- Assessing stonework, forged goods, or structural integrity → **Mason's Tools** proficiency
- Long submersion, smoke, or a held-breath scenario → **Unending Breath** *(RAW: hold breath indefinitely while not Incapacitated)*
