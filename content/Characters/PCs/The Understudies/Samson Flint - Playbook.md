...
*Companion to [[Samson Flint]] — regenerated from the synced Character Sheet on 2026-09-06. Rerun this skill after any character-sheet-sync update to keep it current.*

*Pronouns not set on the PC note — using they/them by default.*

*No Loadout Selection section — Samson isn't a daily-prep caster. Rune Shaper grants one fixed known rune (chosen once, not swapped daily), and its 1st-level spell is only re-inscribed each Long Rest, not re-selected.*

*Which rune Samson knows via Rune Shaper isn't yet chosen in Foundry — pending player choice. Once set, it adds a new Action-table entry and likely a Non-Combat & Creative Uses entry depending on which rune is picked; neither can be built yet.*

## Combat Decision Flowchart

**Triage first — check every turn before anything else:**
- **Self or an ally about to drop, and the one-per-Long-Rest touch heal hasn't been used yet →** **Healing Hands** (touch, 2d4 HP, 1/LR).
- **You just failed an important ability check (not an HP emergency) →** **Tactical Mind**: spend a Second Wind use to roll 1d10 and add it to the check instead of healing — if it still fails, the use isn't spent.
- **Need a full extra turn's worth of actions →** **Action Surge** (self, 1/SR or LR — not the Magic action).
- **A dangerous enemy needs to not reach you or an ally →** **Javelin** *(Slow)* (M/R +6, reduces its Speed by 10 ft. until the start of your next turn).

**Action:**
- **Maximum single-target damage →** **Greatsword** *(Graze)* (M +6, 2d6+4 slash; still deals 4 slash on a miss).
- **Want to blunt this target's next swing back →** **Flail** *(Sap)* (M +6, 1d8+4 bludg.; on hit, target has disadv. on its next attack before the start of your next turn).
- **Ranged, or need to slow something down →** **Javelin** *(Slow)* (see Triage).
- **Echo is deployed and you're taking the Attack action →** also get one free extra melee attack from the echo's position (**Unleash Incarnation**, 3/day, CON mod).

**Bonus Action** (independent of the above):
- **Need eyes/a body in a second spot, or to attack from two angles →** **Manifest Echo** (15 ft., unoccupied space; AC 16, 1 HP, immune to all conditions; command it to move 30 ft./turn free).
- **Need to cross a gap or escape instantly, regardless of actual distance →** **Echo Swap** (teleport-swap with your echo; flat 15 ft. of movement cost no matter how far apart you are).
- **HP is the actual problem →** **Second Wind** (1d10+3 HP, 2/LR, 1 refreshes on Short Rest).
- **Need a temporary combat form for the fight ahead →** **Celestial Revelation** (self, 1 min, choose one: Heavenly Wings (fly speed = walk speed), Inner Radiance (light + 2 radiant dmg/turn to creatures within 10 ft.), or Necrotic Shroud (CHA 13 or Frightened, non-allies within 10 ft.); while active, +2 necrotic or radiant once/turn on a hit; 1/LR).

**Reaction:**
- **A creature you can see within 5 ft. of your echo moves 5+ ft. away from it →** **Echo Opportunity Attack** (M +6, as if attacking from the echo's space).
