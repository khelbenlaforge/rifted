---
created: 2026-06-24T18:54:17

title: "Knights of the Eternal Order"
aliases:
tags:
  - faction
campaign: Rifted
introduced: 2026-06-20
secret: false
updated: 2026-06-24T18:54:17
---

# Knights of the Eternal Order

> [!infobox|right wikipedia]+
> # Knights of the Eternal Order
> ###### Quick Reference
> | | |
> | ---- | --- |
> | Type | Military / Religious (Knightly Order) |
> | Base | [[Temple of Kelemvor]] |
> | Leader | Unknown |

## Description

The Knights of the Eternal Order are the paladins of the [[Temple of Kelemvor]], and their specific charge is the Threshold Gates — three archways in the temple's Knights' hall that open, at certain hours, onto the Abyss, the Nine Hells, and the Outplane. The Knights stand before them at all times. Not to prevent passage. To ensure it goes the right way.

Every cross-planar transit in [[Crystal City]] that runs through legitimate channels runs through the Knights. They authorize outbound trips into the planes. They intercept unsanctioned arrivals. They return the dead to the appropriate destination when something or someone tries to interfere with the process. They are not gatekeepers in the bureaucratic sense — they are the gate itself, standing upright.

The name is a statement of scope: *eternal* because death does not end, and *order* because the Knights are what keeps it from becoming chaos.

## Leadership & Members

The order's internal hierarchy is not publicly displayed. Rank is visible in the trim of the vestment and the number of seals on the sword harness; names are known to those who need to know them.

- **[[Augustus Kennedy]]** — Joining as an initiate. A paladin of [[Kelemvor Lyonsbane|Kelemvor]] by divine signature — the [[Kelemvor's Brand|Brand]] made the affiliation clear before the Knights were consulted about it. He will hold his law enforcement post with [[Hall of Justice - Temple of Tyr|Tyr's judiciary]] concurrently; the temple has noted this and appears unconcerned.

## Operations

The Knights maintain a constant watch rotation at the Threshold Gates. When the Gates are open, they run active assessments of incoming transit — unauthorized arrivals are intercepted immediately, their connection to the appropriate destination determined, and the matter resolved. The resolution is rarely complicated. The Knights have very good instincts for where things belong.

Beyond the gates, the Knights handle incidents of unauthorized extraplanar transit across the city — individuals attempting to travel without clearance, entities arriving through unregistered channels, or cases where the dead are being redirected by an outside party. The last category interests them most.

## Current Status

Operating at full capacity within the [[Temple of Kelemvor]]. The Gates have been active more frequently over recent months — the increase in [[Shadow Creatures]] activity and the general instability of planar transit have kept the watch rotation heavy.

[[Garm]]'s assessment that the Shadow Creatures are extraplanar but not from any recognized plane has reached the Knights through temple channels. They have not confirmed or denied an operational response.

[[Augustus Kennedy]] is joining the order. His background in law enforcement maps cleanly onto the Knights' cross-planar policing function. They are not in the habit of turning away paladins whose Brand is already doing the work.

## Membership

**How to Join:** Invitation only, extended to paladins with a demonstrated divine connection to [[Kelemvor Lyonsbane|Kelemvor]] or to the domain of death. Exceptional martial champions with sponsorship from the temple's upper ranks may petition; the threshold is high.

### Ranks

| Rank | Title | Notes |
|---|---|---|
| 1 | Aspirant | Initiate; shadow assignment; learning gate protocols |
| 2 | Knight-Warden | Full standing; active gate rotation; authorized to intercept |
| 3 | Knight-Sentinel | Senior knight; leads watch rotations; gate authorization authority |
| 4 | Knight-Marshal | Commander; operational authority; sets gate schedules |

### Benefits

**Aspirant** *(Rank 1)*
- Temple backing for law enforcement actions — the name carries weight in Crystal City
- Access to the Knights' hall during off-watch hours; gate observation permitted
- Cross-planar transit authorization for official missions — no waiting, no fee

**Knight-Warden** *(Rank 2)*
- Active gate authorization — can independently clear outbound planar transit
- Ability to formally flag unauthorized extraplanar entities to the temple's response chain
- Proficiency (or expertise if already proficient) in Athletics for purposes of gate containment

---

Leader:: Unknown
Headquarters:: [[Temple of Kelemvor]]


