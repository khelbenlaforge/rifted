---
title: "The Harpers"
aliases:
  - Harpers
tags:
  - faction
campaign: Rifted
introduced: 2026-03-23
secret: false
updated: 2026-08-28T02:33:48
---

# The Harpers

> [!infobox|right wikipedia]+
> # The Harpers
> ![[zzz_Attachments/Harpers Logo.png]]
> ###### Quick Reference
> | | |
> | ---- | --- |
> | Also Known As | Harpers |
> | Headquarters | Distributed — no fixed base |
> | Founded By | [[Storm Silverhand]] *(among others)* |
> | Alignment | Good |
> | Status | Active |

## Description

A secret network of agents, bards, rangers, and scholars spread across Faerûn, working to preserve balance, oppose tyranny, and protect those who cannot protect themselves. The Harpers have no palace, no army, no formal chain of command. Their strength is that they are everywhere — and no one knows who belongs.

The founding doctrine, credited to [[Storm Silverhand]], is absolute: **agents never identify each other in the field.** A Harper who meets another Harper in an active situation acts as if they are strangers, because Harpers *are* strangers to each other unless the mission explicitly requires otherwise. The anonymity is the network. Break it once and you break the chain back to everyone that person has ever worked with.

This is not paranoia. It is discipline. It is how an organization with no walls or walls survives centuries.

## Known Members (Campaign)

- [[Storm Silverhand]] — founding Harper; Chosen of Mystra
- [[Selmavra Elsree]] — Harper skirmish commander, Shadowdale
- [[Torm of the Dales]] — Harper agent; neither he nor Selmavra will say so in the field
- [[Mourngrym Amcathra]] — allied; both Harper-affiliated

## Role in the Story

*Prequel vision — Avatar Crisis era.* Harpers are actively moving through the Shadowdale region, tracking Zhentarim assassins heading south. [[Selmavra Elsree]] is the operational presence in Shadowdale. [[Storm Silverhand]] is present at Elminster's Tower and aware of the situation.

The prequel party includes at least two Harpers (Selmavra/Torm). Neither identifies the other. [[Rathan Thentraver]] notices the Harpers were moving before the party arrived and asks Torm directly who's running the operation — a question JJ has to navigate.

---

Leader:: Distributed *(no single leader)*
Headquarters:: Distributed

